...
void
try_gpu_place(struct s_placer_opts placer_opts,
      struct s_annealing_sched annealing_sched,
      t_chan_width_dist chan_width_dist,
      struct s_router_opts router_opts,
      struct s_det_routing_arch det_routing_arch,
      ...
      t_mst_edge *** mst)
{
    ...
    if(placer_opts.place_algorithm == NET_TIMING_DRIVEN_PLACE ||
       placer_opts.place_algorithm == PATH_TIMING_DRIVEN_PLACE)
    {
        printf("Error: timing placement not supported with gpu.\n");
        exit(1);
    }
    ...
    */
...
#ifndef SPEC
    printf
    ("%11s  %10s %11s %11s  %11s  %11s %11s  %11s %9s %8s  %7s  %7s  %10s  %7s\n",
     "T", "Cost", "Av. BB Cost", "GPU BB Cost", "Av. TD Cost", "Av Tot Del",
     "P to P Del", "d_max", "Ac Rate", "Std Dev", "R limit", "Exp",
     "Tot. Moves", "Alpha");
    printf
    ("%11s  %10s %11s %11s  %11s  %11s %11s  %11s %9s %8s  %7s  %7s  %10s  %7s\n",
    ...
    update_screen(MAJOR, msg, PLACEMENT, FALSE);
    //printf("Wait..."); getchar(); printf("STEP\n");


    //update_screen(MINOR, msg, PLACEMENT, FALSE);
    //printf("Wait..."); getchar(); printf("STEP\n");

    // Copy data to gpu before starting
    gpu_place_copy_data_to_gpu(num_types, type_descriptors,
                        grid, nx, ny,
                        block, num_blocks,
                        clb_net, net_cost, num_nets,
                        chanx_place_cost_fac, chany_place_cost_fac);

    // Here's a hack to control number of kernel launches
    // TODO: make this an option
    const int max_kernel_launches = 1;
    int kernel_launches_done = 0;
    ...
    while(exit_crit(t, cost, annealing_sched) == 0)
    {
        ...
        */
        ...
        // LAUNCH KERNEL for single temperature
        gpu_place_launch_kernel(nx, ny, num_blocks, num_nets,
                                t, rlim, &move_lim, &success_sum);

        kernel_launches_done += 1;

        //double gpu_bb_cost = bb_cost;

        // Copy net_cost back to vpr
        gpu_update_net_costs_on_vpr(net_cost, num_nets);
        // Recompute bb_cost using GPU's net_cost
        double gpu_bb_cost = recompute_bb_cost(placer_opts.place_cost_type, placer_opts.num_regions);
        bb_cost = gpu_bb_cost;

        /*
        // Recompute from scratch the net_cost and bb_cost on VPR, and copy back the updated net costs
        // Copy all the data back to vpr
        gpu_place_copy_data_to_vpr(num_types, type_descriptors,
                            grid, nx, ny,
                            block, num_blocks,
                            clb_net, net_cost, num_nets);
        bb_cost = comp_bb_cost(NORMAL, placer_opts.place_cost_type,
                placer_opts.num_regions);
        gpu_update_net_costs_on_gpu(net_cost, num_nets);
        */
...
*/
...
#ifndef SPEC
        printf
        ("%11.5g  %10.6g %11.6g %11.6g  %11.6g  %11.6g %11.6g %11.4g %9.4g %8.3g  %7.4g  %7.4g  %10d  ",
         t, av_cost, av_bb_cost, gpu_bb_cost, av_timing_cost, av_delay_cost,
         place_delay_value, d_max, success_rat, std_dev, rlim,
         crit_exponent, tot_iter);
#endif
        ...
        */
        ...
        if(kernel_launches_done >= max_kernel_launches)
            break;
    }
    ...
    gpu_place_copy_data_to_vpr(num_types, type_descriptors,
                        ...
                        block, num_blocks,
                        clb_net, net_cost, num_nets);
    // Recompute net_cost and bb_cost from scratch
    bb_cost = comp_bb_cost(NORMAL, placer_opts.place_cost_type, placer_opts.num_regions);
    ...
    */
    ...
    printf
    ("%11.5g  %10.6g %11.6g %11.6g  %11.6g  %11.6g %11.6g %11.4g %9.4g %8.3g  %7.4g  %7.4g  %10d  \n\n",
     t, av_cost, av_bb_cost, 0.0, av_timing_cost, av_delay_cost,
     place_delay_value, d_max, success_rat, std_dev, rlim,
     crit_exponent, tot_iter);
