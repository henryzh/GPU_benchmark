...
BFS_kernel_multi_blk_inGPU(int *q1,
  ...
  int kt = atomicOr(global_kt,0);// the total count of GPU global barrier
...
//    start_global_barrier(kt+1);
    __barrier_atomic(NUM_SM*(kt+1));
#endif
#ifdef LOCKFREE
    __barrier_lockfree(kt+1, in, out);
#endif
...
//    start_global_barrier(kt+2);
    __barrier_atomic(NUM_SM*(kt+2));
#endif
#ifdef LOCKFREE
    __barrier_lockfree(kt+2, in, out);
#endif
