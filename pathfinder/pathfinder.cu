...
int calc_path(int *gpuWall, int *gpuResult[2], int rows, int cols, int pyramid_height, int blockCols, int borderCols) {
    ...
    for (int t = 0; t < rows-1; t+=pyramid_height) { 
       ...
       src = dst;
       dst = temp;
       int iter = MIN(pyramid_height, rows-t-1);
 //      printf("[BENCH] iteration=%d\n", iter);
       dynproc_kernel<<<dimGrid, dimBlock>>>(iter, gpuWall, gpuResult[src], gpuResult[dst], cols, rows, t, borderCols);
    }
    return dst;
}
...
void run(int argc, char** argv) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy(result, gpuResult[final_ret], sizeof(int)*cols, cudaMemcpyDeviceToHost));
